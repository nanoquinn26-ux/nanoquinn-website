      ...
      <div className="bg-foreground text-background py-24 px-6">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-background/60 text-lg leading-relaxed max-w-2xl mx-auto">
            ...
            We are building a team of <span className="text-[#25D366] font-semibold">category creators</span>.
          </p>
          ...
          <ul className="grid sm:grid-cols-2 gap-4">
            {whyItems.map((item) => (
              <li key={item} className="flex items-start gap-3 bg-muted rounded-xl p-4">
                <span className="mt-1 w-2 h-2 rounded-full bg-[#25D366] shrink-0" />
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-8">
            {whoWeWant.map(({ icon: Icon, text }) => (
              <div key={text} className="bg-background border border-border rounded-2xl p-5 flex items-start gap-4 hover:border-[#25D366]/60 transition-colors">
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {whatYoullBuild.map(({ icon: Icon, text }) => (
              <div key={text} className="flex items-start gap-4 p-5 rounded-2xl border border-border hover:border-[#25D366]/50 transition-colors">
                <div className="w-9 h-9 rounded-full bg-[#25D366]/10 flex items-center justify-center shrink-0">
                  <Icon className="w-4 h-4 text-[#25D366]" />
                </div>
                <p className="text-sm text-foreground leading-relaxed">{text}</p>
      ...
      <div className="py-24 px-6 bg-foreground text-background text-center">
        <div className="max-w-2xl mx-auto">
          ...
          <a
            ...
            className="inline-flex items-center gap-2 mt-8 px-8 py-4 rounded-full bg-[#25D366] text-black font-bold text-base hover:opacity-90 transition-opacity"
          >
            hr@nanoquinn.com
